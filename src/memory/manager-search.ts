  ...
  const rows = params.db
    .prepare(
      `SELECT id, path, source, start_line, end_line, text,\n` +
        ...
        ` WHERE ${params.ftsTable} MATCH ?${modelClause}${params.sourceFilter.sql}${pf?.sql ?? ""}\n` +
        ` ORDER BY rank ASC\n` +
        ` LIMIT ?`,
    )
    .all(ftsQuery, ...modelParams, ...params.sourceFilter.params, ...(pf?.params ?? []), params.limit) as Array<{
    id: string;
    path: string;
    source: SearchSource;
    start_line: number;
