    ...
    wrapToolWithAudit(
      {
        ...
        async execute(_id: string, params: Record<string, unknown>) {
          try {
            ...
            const result = await scopelyFetch(`/api/extraction-monitor/metrics/${qs}`);
            if (!result.ok) {
              return jsonResult({
                ok: false,
    ...
    wrapToolWithAudit(
      {
        ...
        async execute(_id: string, params: Record<string, unknown>) {
          try {
            ...
            const result = await scopelyFetch(`/api/extraction-monitor/sessions/${qs}`);
            if (!result.ok) {
              return jsonResult({
                ok: false,
                error: `HTTP ${result.status}`,
        ...
        async execute(_id: string, params: Record<string, unknown>) {
          try {
            const result = await scopelyFetch(
              `/api/extraction-monitor/sessions/${encodeURIComponent(params.session_id as string)}/`,
            );
            if (!result.ok) {
              return jsonResult({
                ok: false,
