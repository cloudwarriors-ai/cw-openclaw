...
async function bhDevtoolsFetch(
  ...
  if (!token) {
    return { ok: false, status: 0, data: { error: "BIGHEAD_DEV_TOOLS_API env var not set" } };
  }
...
export function registerDevtoolsTools(api: OpenClawPluginApi, logger: AuditLogger) {
  // bh_devtools_list_containers
  api.registerTool(() =>
    wrapToolWithAudit(
      {
        name: "bh_devtools_list_containers",
        description:
          "List all Docker containers on the Bighead server with their name, image, state, and status.",
  ...
  // bh_devtools_get_logs
  api.registerTool(() =>
    wrapToolWithAudit(
      {
        name: "bh_devtools_get_logs",
        description:
  ...
  // bh_devtools_db_tables
  api.registerTool(() =>
    wrapToolWithAudit(
      {
        name: "bh_devtools_db_tables",
        description:
          "List all tables in the public schema of the Bighead database.",
        parameters: Type.Object({}),
        async execute() {
          try {
            const result = await bhDevtoolsFetch("/api/v1/db/tables");
            if (!result.ok) return jsonResult({ ok: false, error: `HTTP ${result.status}`, details: result.data });
  ...
  // bh_devtools_db_table_schema
  api.registerTool(() =>
    wrapToolWithAudit(
      {
        name: "bh_devtools_db_table_schema",
        description:
          "Get the column definitions for a Bighead database table.",
        parameters: Type.Object({
        ...
        async execute(_id: string, params: Record<string, unknown>) {
          try {
            ...
            const result = await bhDevtoolsFetch(`/api/v1/db/tables/${encodeURIComponent(tableName)}/schema`);
            if (!result.ok) return jsonResult({ ok: false, error: `HTTP ${result.status}`, details: result.data });
  ...
  // bh_devtools_db_query
  api.registerTool(() =>
    wrapToolWithAudit(
      {
        name: "bh_devtools_db_query",
        description:
          "Execute a read-only SQL query (SELECT or WITH only) against the Bighead database. Returns up to 1000 rows.",
        parameters: Type.Object({
