...
const BH_EMAIL = () => process.env.BIGHEAD_PROJECT_PULSE_EMAIL ?? process.env.PROJECT_PULSE_EMAIL ?? "";
const BH_PASSWORD = () => process.env.BIGHEAD_PROJECT_PULSE_PASSWORD ?? process.env.PROJECT_PULSE_PASSWORD ?? "";
...
export async function bhLogin(): Promise<string> {
  const baseUrl = BH_URL();
...
export async function bhGetSession(): Promise<string> {
  if (cachedCookie && Date.now() < cookieExpiry) return cachedCookie;
  return bhLogin();
}

export function bhClearSession() {
  cachedCookie = null;
  cookieExpiry = 0;
}
